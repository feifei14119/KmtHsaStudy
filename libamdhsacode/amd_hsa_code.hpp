...
template<uint64_t signature> class Signed 
{
public:
...
template<uint64_t signature> const uint64_t Signed<signature>::CT_SIGNATURE = signature;

bool IsAccessibleMemoryAddress(uint64_t address);

template<typename class_type, typename member_type> size_t OffsetOf(member_type class_type::*member)
{
  return (char*)&((class_type*)nullptr->*member) - (char*)nullptr;
}

template<typename class_type> class_type * ObjectAt(uint64_t address)
{
...
namespace code 
{
	...
	{
    public:
      explicit VariableSymbol(amd::elf::Symbol* elfsym_) : Symbol(elfsym_) { }
    ...
    public:
      ...
      Section* AddDataSection(const std::string &name, uint32_t type, uint64_t flags, Segment* segment);
      ...
      Symbol* AddExecutableSymbol(const std::string &name, unsigned char type, unsigned char binding, unsigned char other, Section *section = 0);

      Symbol* AddVariableSymbol(const std::string &name, unsigned char type, unsigned char binding, unsigned char other, Section *section, uint64_t value, uint64_t size);
