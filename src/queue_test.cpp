...
void allocate_ring_buffer()
{
	...
	ring_buffer = AllocMemoryCPU(queue_size);
...
void KmtCreateQueue(uint32_t queue_type, void * ring_buff, uint32_t ring_size, HsaQueueResource * queuer_rsc)
{
	...
	// malloc kmt_queue_t ----------------------------------------------
	kmt_queue_t * kmt_queue = (kmt_queue_t*)AllocMemoryCPU(sizeof(kmt_queue_t));
	...
	printf("\t\tkmt queue address = 0x%016lX.\n", kmt_queue);
	printf("\t\tkmt queue size = %d(Byte).\n", sizeof(kmt_queue_t));

	// malloc eop buffer ----------------------------------------------
	kmt_queue->eop_buffer = AllocMemoryGPUVM(vega20_eop_buff_size);
	printf("\n\t-----------------------\n");
	...
	kmt_queue->ctx_save_restore = AllocMemoryCPU(kmt_queue->ctx_save_restore_size);
	printf("\n\t-----------------------\n");
	...
	args.gpu_id = gGpuId;
	args.queue_priority = 0; // HSA_QUEUE_PRIORITY_NORMAL
	...
	uint64_t doorbell_mmap_offset = args.doorbell_offset & ~(uint64_t)(doorbell.size - 1);
	uint32_t doorbell_offset = args.doorbell_offset & (doorbell.size - 1);
	void * doorbell_addr = AllocMemoryDoorbell(doorbell.size, doorbell_mmap_offset);
	doorbell.mapping = doorbell_addr;
	printf("\n\t-----------------------\n");
	printf("\tmallocate doorbell:\n");
...
void QueueTest()
{
	...
	hsa_queue_t * user_queue; // to user
